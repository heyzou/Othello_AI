"""exp_047_iterative_deepening.py: Iterative Deepening + TT Best Move Priority + Hybrid Move Ordering. (Reference / Non-submission experimental code)"""
...
class MyPlayer(BasePlayer):
    ...
    def _negascout(
        ...
        if depth >= 2 and len(children) > 1:
            ...
            MyPlayer._accum_search_moveorder_time += time.perf_counter() - t0_sort
    ...
    def _search_hash_index(cls, key: tuple[int, int, int, bool]) -> int:
        black_bits, white_bits, color, allow_probcut = key
        value = black_bits ^ ((white_bits << 1) & cls.FULL_MASK)
        value ^= color * 131 + int(allow_probcut)
    ...
    def _log_profile(self, total_time: float, is_book: bool, actual_turn: int):
        ...
        s_movegen_time = getattr(MyPlayer, "_accum_search_movegen_time", 0.0)
        s_applymove_time = getattr(MyPlayer, "_accum_search_applymove_time", 0.0)
        s_moveorder_time = getattr(MyPlayer, "_accum_search_moveorder_time", 0.0)
        s_tthash_time = getattr(MyPlayer, "_accum_search_tthash_time", 0.0)

        book_time = getattr(self, "_t_book", 0.0)
        search_total = getattr(self, "_t_search", 0.0)
        search_other = max(0.0, search_total - eval_time)

        s_sub_sum = s_movegen_time + s_applymove_time + s_moveorder_time + s_tthash_time
        s_control_time = max(0.0, search_other - s_sub_sum)
        ...
        if search_other > 0:
            gen_pct = (s_movegen_time / search_other) * 100
            app_pct = (s_applymove_time / search_other) * 100
            ord_pct = (s_moveorder_time / search_other) * 100
            tth_pct = (s_tthash_time / search_other) * 100
            ctl_pct = (s_control_time / search_other) * 100
            search_sub_str = f"         ├─ [Search Breakdown] MoveGen: {s_movegen_time * 1000:7.2f} ms ({gen_pct:5.1f}%) | ApplyMove: {s_applymove_time * 1000:7.2f} ms ({app_pct:5.1f}%) | MoveOrder: {s_moveorder_time * 1000:7.2f} ms ({ord_pct:5.1f}%) | TTHash: {s_tthash_time * 1000:7.2f} ms ({tth_pct:5.1f}%) | Control: {s_control_time * 1000:7.2f} ms ({ctl_pct:5.1f}%)\n"
            log_str += search_sub_str
        ...
        print(log_str, end="")
        log_file = "exp_047_profile_log.txt"
