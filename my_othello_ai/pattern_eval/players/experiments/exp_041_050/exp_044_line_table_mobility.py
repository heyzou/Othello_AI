"""exp_044_line_table_mobility.py: Precomputed 1D Row/Column Table Lookup for Mobility. (Reference / Non-submission experimental code)"""
...
class MyPlayer(BasePlayer):
    ...
    # Precomputed 1D Line Legal Moves Lookup Table (65,536 entries = 64KB)
    LINE_LEGAL_MOVES_TABLE = None

    @classmethod
    def _get_line_legal_moves_table(cls):
        if cls.LINE_LEGAL_MOVES_TABLE is not None:
            return cls.LINE_LEGAL_MOVES_TABLE
        table = [0] * 65536
        for own in range(256):
            for opp in range(256):
                legal = 0
                empty = (~(own | opp)) & 0xFF
                for pos in range(8):
                    if not ((empty >> pos) & 1): continue
                    # Left (-1)
                    p, flips = pos - 1, 0
                    while p >= 0 and ((opp >> p) & 1): flips += 1; p -= 1
                    if flips > 0 and p >= 0 and ((own >> p) & 1): legal |= (1 << pos); continue
                    # Right (+1)
                    p, flips = pos + 1, 0
                    while p < 8 and ((opp >> p) & 1): flips += 1; p += 1
                    if flips > 0 and p < 8 and ((own >> p) & 1): legal |= (1 << pos)
                table[(own << 8) | opp] = legal
        cls.LINE_LEGAL_MOVES_TABLE = tuple(table)
        return cls.LINE_LEGAL_MOVES_TABLE

    @staticmethod
    def _transpose_64(b: int) -> int:
        t = 0x00AA00AA00AA00AA & (b ^ (b >> 7))
        b = b ^ t ^ (t << 7)
        t = 0x0000CCCC0000CCCC & (b ^ (b >> 14))
        b = b ^ t ^ (t << 14)
        t = 0x00000000F0F0F0F0 & (b ^ (b >> 28))
        b = b ^ t ^ (t << 28)
    ...
    def _legal_move_mask_bits(self, state: tuple[int, int], color: Cell) -> int:
        ...
        table = MyPlayer._get_line_legal_moves_table()

        # 1. Horizontal Rows via 1D Table Lookup
        legal = table[(own & 0xFF) << 8 | (opp & 0xFF)]
        legal |= table[((own >> 8) & 0xFF) << 8 | ((opp >> 8) & 0xFF)] << 8
        legal |= table[((own >> 16) & 0xFF) << 8 | ((opp >> 16) & 0xFF)] << 16
        legal |= table[((own >> 24) & 0xFF) << 8 | ((opp >> 24) & 0xFF)] << 24
        legal |= table[((own >> 32) & 0xFF) << 8 | ((opp >> 32) & 0xFF)] << 32
        legal |= table[((own >> 40) & 0xFF) << 8 | ((opp >> 40) & 0xFF)] << 40
        legal |= table[((own >> 48) & 0xFF) << 8 | ((opp >> 48) & 0xFF)] << 48
        legal |= table[((own >> 56) & 0xFF) << 8 | ((opp >> 56) & 0xFF)] << 56

        # 2. Vertical Columns via Transpose + Table Lookup
        own_t = MyPlayer._transpose_64(own)
        opp_t = MyPlayer._transpose_64(opp)
        col_legal = table[(own_t & 0xFF) << 8 | (opp_t & 0xFF)]
        col_legal |= table[((own_t >> 8) & 0xFF) << 8 | ((opp_t >> 8) & 0xFF)] << 8
        col_legal |= table[((own_t >> 16) & 0xFF) << 8 | ((opp_t >> 16) & 0xFF)] << 16
        col_legal |= table[((own_t >> 24) & 0xFF) << 8 | ((opp_t >> 24) & 0xFF)] << 24
        col_legal |= table[((own_t >> 32) & 0xFF) << 8 | ((opp_t >> 32) & 0xFF)] << 32
        col_legal |= table[((own_t >> 40) & 0xFF) << 8 | ((opp_t >> 40) & 0xFF)] << 40
        col_legal |= table[((own_t >> 48) & 0xFF) << 8 | ((opp_t >> 48) & 0xFF)] << 48
        col_legal |= table[((own_t >> 56) & 0xFF) << 8 | ((opp_t >> 56) & 0xFF)] << 56
        legal |= MyPlayer._transpose_64(col_legal)

        # 3. Diagonals via Inlined Shifts
        FULL_MASK = self.FULL_MASK
        NOT_A = self.NOT_A_FILE
        NOT_H = self.NOT_H_FILE
        empty = FULL_MASK ^ (own | opp)
    ...
    def _log_profile(self, total_time: float, is_book: bool, actual_turn: int):
        ...
        print(log_str, end="")
        log_file = "exp_044_profile_log.txt"
