"""Opening book data and helper utilities."""
    ...
    for line in BOOK_LINES:
        ...
        for transform in range(4):
            ...
            for i in range(0, len(prefix), 2):
                move = coord_to_move(prefix[i:i + 2], transform)
                board = apply_move_for_color(board, move, current_color)
                current_color = Cell.WHITE if current_color == Cell.BLACK else Cell.BLACK
            cache[(board_key(board), current_color)] = coord_to_move(move_text, transform)

    BOOK_CACHE = cache
    return cache


def book_move_bits(state: tuple[int, int], color: Cell) -> Move | None:
    return get_book_cache().get((board_key_bits(state), color))
...
def coord_to_move(coord: str, transform: int) -> Move:
...
def apply_move_for_color(board: Board, move: Move, color: Cell) -> Board:
    row, col = move
...
def board_key(board: Board) -> tuple[int, ...]:
    values = []
    for row in board:
        for cell in row:
            if cell == Cell.BLACK:
...
def board_key_bits(state: tuple[int, int]) -> tuple[int, ...]:
